...
const getCurrentData = function (name, forecastData) {
  return {
    ...
    date: getFormattedData(forecastData.current.dt, "ddd DD/MM/YYYY HH:mm"),
...
const renderCurrentWeatherCard = function (currentData) {
  const currentWeatherCard = `<div class="card-body bg-white border mb-2">
        <h2 class="card-title">${currentData.name} ${
    currentData.date
    ...
    currentData.iconCode
        ...
        <p class="card-text">UV index: <span class="btn ${getUVIClassName(
          currentData.uvi
        )}">${currentData.uvi}</span></p>
        </div>`;
